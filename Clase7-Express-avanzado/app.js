...
app.post("/api/user", (req, res) => {
  ...
  if (!user.first_name || !user.last_name) {
    return res
      .status(400)
      .send({ status: `error`, error: `Informacion incompleta` });
  }
...
app.put("api/user/:name", (req, res) => {
  ...
  if (user.first_name === name) {
    return res
      ...
      .send({
        status: "error",
        error: "La informacion no coincide con lo que esta guardando",
      });
  }
  users.push(user);
  res.send({ status: "succes", message: "Actualizado" });
});
...
app.delete("/api/user/:name", (req, res) => {
  ...
  res.send({ status: "succes", message: "Borrado" });
});
